...
NARRATIVES_PATH = os.path.join(BASE_DIR, "narratives.json")
WINDOWED_SCREEN_SIZE = (1280, 720)
GAME_TITLE = "BRUCE'S Treasure"
DEBUG = False
...
def load_sound(name):
    ...
    if os.path.exists(path):
        try:
            ...
            print(f"Warning: could not load sound {name}: {exc}")
    return None

MUSIC_PLAYLIST = ["music.ogg", "01.ogg", "02.ogg", "03.ogg", "04.ogg", "05.ogg", "06.ogg", "07.ogg", "08.ogg"]
SONG_FINISHED = None
...
def play_next_song():
    ...
    playlist = [song for song in MUSIC_PLAYLIST if not IS_BROWSER or song.lower().endswith(".ogg")]
    if not playlist:
        print("No songs in playlist.")
        return

    next_song = random.choice(playlist)
    music_path = os.path.join(ASSET_PATH, next_song)
    ...
    try:
        ...
        print(f"Warning: could not play {next_song}: {exc}")
...
def load_sprite(name):
    ...
    if os.path.exists(path):
        try:
            ...
            print(f"Warning: could not load sprite {name}: {exc}")
    return None
...
class TNTSystem:
    ...
    def stop_all_fuse_sounds(self):
        for charge in self.charges.values():
            channel = charge.get("fuse_channel")
            if channel:
                channel.stop()
                charge["fuse_channel"] = None

    def rebuild(self, grid):
        self.stop_all_fuse_sounds()
        self.charges = {}
        for y in range(WORLD_ROWS):
            for x in range(WORLD_COLS):
                if grid[y][x] == TNT:
    ...
    def update(self, delta_time, player, grid, particles, spike_system, fireball_system):
        ...
        for (x, y), charge in list(self.charges.items()):
            ...
            if not charge["lit"] and distance <= TNT_TRIGGER_RADIUS:
                ...
                if SND_TNT_IGNITE:
                    charge["fuse_channel"] = SND_TNT_IGNITE.play()
    ...
    def explode(self, x, y, player_x, player_y, player_health, grid, particles, spike_system, fireball_system):
        ...
        charge = self.charges.pop((x, y), None)
        if charge:
            channel = charge.get("fuse_channel")
            if channel:
                channel.stop()
...
def load_narratives():
    try:
        ...
        print(f"Warning: {NARRATIVES_PATH} not found or invalid")
        return []
...
def initialize_runtime(browser=False):
    ...
    global clock, font, big_font, huge_font, SONG_FINISHED
    global SND_SPIKE, SND_PICKUP, SND_EXTEND, SND_WIN, SND_TRAP_FALL, SND_EXTRA_LIFE
    global SND_FALL, SND_SPIKE_HIT, SND_TNT_IGNITE, SND_TNT_BOOM
    global SPRITES, PLAYER_SPRITE, PLAYER_BIG_SPRITE
    global NARRATIVE_BACKGROUNDS, current_narrative_bg, LIFE_LOSS_BACKGROUNDS, FALL_BACKGROUNDS
    global current_message_bg, current_message_bg_img, message_bg_initialized
    global state, grid, player, spike_system, fireball_system, tnt_system, particles
    global last_time, last_level_score, fall_start_time, fall_duration, death_type
    global narratives, current_narrative, message_text, message_type, message_start_time, message_duration
    global persistent_level, persistent_score, persistent_lives, persistent_health, persistent_collected
    global camera_x, camera_y

    IS_BROWSER = browser or sys.platform == "emscripten"
    ...
    if IS_BROWSER:
        ...
        width = display_info.current_w or WINDOWED_SCREEN_SIZE[0]
        height = display_info.current_h or WINDOWED_SCREEN_SIZE[1]
        width = min(width, WINDOWED_SCREEN_SIZE[0])
        height = min(height, WINDOWED_SCREEN_SIZE[1])
    ...
    SND_SPIKE = load_sound("hit.ogg")
    SND_PICKUP = load_sound("coin.ogg")
    SND_EXTEND = load_sound("spike_extend.ogg") or SND_SPIKE
    SND_WIN = load_sound("win.ogg") or SND_PICKUP
    SND_TRAP_FALL = load_sound("trap_fall.ogg") or SND_SPIKE
    SND_EXTRA_LIFE = load_sound("extra_life.ogg")
    SND_FALL = load_sound("fall.ogg")
    SND_SPIKE_HIT = load_sound("spikehit.ogg")
    SND_TNT_IGNITE = load_sound("tnt_ignite.ogg") or SND_EXTEND
    SND_TNT_BOOM = load_sound("tnt_explode.ogg") or SND_SPIKE_HIT
    ...
    if SPRITES.get(TRAP):
        ...
        dark_trap = pygame.Surface((trap_img.get_width(), trap_img.get_height()), pygame.SRCALPHA)
...
def handle_keydown(event):
    global state, current_narrative, persistent_level
...
def update_play_state(delta_time):
    global state, death_type, fall_start_time, persistent_lives
...
def draw_current_state():
    if state == STATE_SPLASH:
        ...
        msg_color = COLOR_GREEN if "climbed out" in message_text.lower() else COLOR_RED
...
async def run_game(browser=False):
    global last_time

    initialize_runtime(browser)
    running = True
    ...
    while running:
        ...
        for event in pygame.event.get():
            if SONG_FINISHED is not None and event.type == SONG_FINISHED:
                play_next_song()
    ...
    if pygame.get_init() and not IS_BROWSER:
        pygame.quit()

def run_desktop():
    asyncio.run(run_game(browser=False))

async def run_browser():
    await run_game(browser=True)
